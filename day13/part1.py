...
def main(points: list[str], folds: list[str]) -> int:
    coords = set()
    for line in points:
        x_s, y_s = line.split(",")
        x, y = int(x_s), int(y_s)
...
def print_points(coords: set[tuple[int, int]]) -> None:
...
def test_main_example_data():
    assert main(*parse_input(INPUT)) == 17


def test_main_real_data():
    input_data = parse_input_file()
    assert main(*parse_input(input_data)) == 592
